...
def create_sample(base_file,n = 500000):
...
def main():
    ...
    for base in range(4):
        base_file = "%ss-%d.csv"%(g_ds_base_path,base)
        base_name =  os.path.splitext(os.path.basename(base_file))[0]
        for i in range(4):
            for j in range(4):
                n = random.randint(50000, 500000)
                print(base_file,i,j,n)
                df = create_sample(base_file,n)
                df.to_csv(g_ds_sample_path + "%s-%d-%d.csv"%(base,i,j))
