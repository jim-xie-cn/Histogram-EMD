...
def filter_dataset(df , kind ,normal_ratio,rate = 0.2):
    #normal sampling
    if kind == 0 :
        if df.shape[0] >= 10000:
    ...
    if kind == 1:
        return get_sample(df,rate,normal_ratio*abnormal_rate,normal_rate,False)

    #banlance sampling ( weighted sampling + replace sampling )
    if kind == 2:
        return get_sample(df,rate,normal_ratio*abnormal_rate,normal_rate,True)

    #more normal ( weighted sampling + replace sampleing )
    if kind == 3:
        return get_sample(df,rate,normal_ratio * abnormal_rate,normal_rate,True)

def create_dataset( kind ,abnormal_ratio, rate= 0.2 ):
    csv_files =[]
    if kind == 0:
    ...
    for fi in csv_files:
        ...
        df_tmp = filter_dataset(df_tmp,kind,abnormal_ratio,rate)
        df_all = pd.concat([df_all,df_tmp], ignore_index = True )
    df_all.reset_index(drop = True)
    return df_all
    df_all.to_csv("%s/b-%d.csv"%(g_ds_base_path,kind))
...
def main():
    ...
    for kind in range(4):
        for i in range(4):
            normal_ratio = random.uniform(0.5, 3)
            fac_ratio = random.uniform(0.1, 0.25)
            print(kind,i,normal_ratio,fac_ratio)
            df_all = create_dataset(kind,normal_ratio,fac_ratio)
            df_all.to_csv("%s/b-%d-%d.csv"%(g_ds_base_path,kind,i))
